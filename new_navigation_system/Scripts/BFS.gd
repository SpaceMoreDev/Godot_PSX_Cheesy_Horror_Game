...
# I need a script to generate spheres all over the map (one level only)
# use BFS with the sphere points to navigate through the map.


func _process(delta):
	if Input.is_action_just_pressed("click"):
		var mouse_pos = global_position_to_tilemap_pos(get_viewport().get_mouse_position())
